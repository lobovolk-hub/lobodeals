...
import { FEATURED_STORE_OPTIONS, getStoreName } from '@/lib/storeMap'
...
function GamesPageContent() {
  ...
  const pageDescription =
    sort === 'best'
      ? 'Browse the strongest overall deals right now.'
      : sort === 'top-rated'
      ? 'Browse deals for games with the best review scores.'
      : sort === 'biggest-discount'
      ? 'Browse the highest discounts available right now.'
      : sort === 'latest'
      ? 'Browse the latest deals loaded into the catalog.'
  ...
  {FEATURED_STORE_OPTIONS.map((store) => (
    <PriceChip
      ...
      onClick={() => router.push(buildUrl(1, { store: store.value }))}
    />
  ))}
</div>
        </div>
