...
import { storeStaticParams, stores } from '../lib/stores.ts'
...
test('all ten canonical profiles use one shared store-themed hero', async () => {
  ...
  assert.match(hero, /getStoreVisualTreatment\(store\.slug\)/)
...
test('profile hero integrates accessible platform links and the official CTA', async () => {
  const hero = await source('components/store-profile-hero.tsx')
...
test('profiles remove internal market and redundant detail UI', async () => {
  const page = await source('app/services/[slug]/page.tsx')
  const hero = await source('components/store-profile-hero.tsx')
  ...
  assert.doesNotMatch(
    publicProfile,
    /Digital scope|Tracked market|United States|US market|market scope|source health|canonical store|tracking status/i
  )
  assert.doesNotMatch(publicProfile, /Store details|store-details-heading|<dl\b|<dt\b|<dd\b/)
  assert.doesNotMatch(page, /store\.(?:digitalScope|marketScope)/)
})
...
test('Microsoft stays canonical with Xbox identity and Rockstar keeps its fallback', async () => {
