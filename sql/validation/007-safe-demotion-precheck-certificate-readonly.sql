-- LoboDeals 3.2 migration 007 precertification.
...
v1_function as materialized (
  select
    ...
    pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(pg_catalog.pg_get_functiondef(procedure.oid), 'UTF8')), 'hex') as definition_sha256,
  ...
  where namespace.nspname = 'public'
    and procedure.proname = 'apply_psdeals_ended_deals_v1'
...
checks(check_id, check_name, severity, passed, observed, expected) as (
  ...
  union all select 3, 'migration_007_absent', 'blocker', history.migration_007_count = 0,
    jsonb_build_object('migration_007_count', history.migration_007_count), jsonb_build_object('migration_007_count', 0)
  from migration_history as history
  ...
  from v1_function
  union all select 15, 'v1_security_and_acl', 'blocker',
    count(*) = 1 and bool_and(owner_name = 'postgres' and security_definer and proconfig = array['search_path=""']::text[] and service_role_execute and not anon_execute and not authenticated_execute),
    jsonb_build_object('count', count(*), 'owner', min(owner_name), 'service_role_execute', bool_and(service_role_execute), 'anon_execute', bool_or(anon_execute), 'authenticated_execute', bool_or(authenticated_execute)),
    jsonb_build_object('count', 1, 'owner', 'postgres', 'service_role_execute', true, 'anon_execute', false, 'authenticated_execute', false)
  from v1_function
  union all select 16, 'v2_absent', 'blocker',
    to_regprocedure('public.apply_psdeals_ended_deals_v2(uuid,uuid,text,text,text,text,text,bigint[],integer,timestamp with time zone)') is null,
    jsonb_build_object('v2_oid', to_regprocedure('public.apply_psdeals_ended_deals_v2(uuid,uuid,text,text,text,text,text,bigint[],integer,timestamp with time zone)')),
    jsonb_build_object('v2_oid', null)
  union all select 17, 'lifecycle_function_count', 'blocker', functions.exact_count = 9,
    to_jsonb(functions), jsonb_build_object('exact_count', 9)
  from lifecycle_functions as functions
  union all select 18, 'lifecycle_function_security', 'blocker', functions.secured_count = 9,
    to_jsonb(functions), jsonb_build_object('secured_count', 9)
  from lifecycle_functions as functions
  union all select 19, 'candidates_and_minima_empty', 'blocker',
  ...
  from operational_counts as counts
  union all select 20, 'monthly_and_cache_observable', 'informational',
  ...
  from operational_counts as counts
  union all select 21, 'no_target_lock_waiters', 'blocker', locks.waiting_locks = 0,
    to_jsonb(locks), jsonb_build_object('waiting_locks', 0)
  from lock_summary as locks
  union all select 22, 'no_relevant_active_clients', 'blocker', activity.relevant_active_backends = 0,
    to_jsonb(activity), jsonb_build_object('relevant_active_backends', 0)
  from activity_summary as activity
  union all select 23, 'migration_007_compatibility', 'blocker',
    context.current_user_name = 'postgres' and history.migration_007_count = 0
      and counts.cycles = 0 and counts.receipts = 0
      and stage.present_count = stage.expected_count
      and monthly.present_count = monthly.expected_count
      and cycle.present_count = cycle.expected_count
      and receipts.present_count = receipts.expected_count,
    jsonb_build_object('current_user', context.current_user_name, 'cycles', counts.cycles, 'receipts', counts.receipts),
    jsonb_build_object('current_user', 'postgres', 'cycles', 0, 'receipts', 0)
  from certificate_context as context
  cross join migration_history as history
  cross join operational_counts as counts
  cross join stage_columns as stage
  cross join monthly_columns as monthly
  cross join cycle_columns as cycle
  cross join receipt_columns as receipts
)
select
