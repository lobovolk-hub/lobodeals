...
test('CLI exposes all safe commands against a fixture workspace', async () => {
  ...
  const validationNow = new Date(Date.now() + 60_000).toISOString()
  assert.equal(await runPsdealsCycleCli(['verify', `--workspace=${value.root_dir}`, `--now=${validationNow}`], io), 0, error)
  assert.equal(await runPsdealsCycleCli(['assemble', `--workspace=${value.root_dir}`, `--now=${validationNow}`], io), 0, error)
